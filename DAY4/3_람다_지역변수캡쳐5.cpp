...
int main()
{
	...
	auto f3 = [=](int a) { ... }; // 모든 지역변수 값 캡쳐. 쓰지말라고 권고한다.
	auto f4 = [&](int a) { ... }; // 모든 지역변수를 reference로 캡쳐
								// => 단, 사용하지않으면 컴파일러최적화로
								//	  람다 클래스의 멤버 변수로 추가 안함

	auto f5 = [=, &v2](int a) { ... }; // v2는 reference, 나머지는 값
	auto f6 = [&,  v2](int a) { ... };
	auto f7 = [&, &v2](int a) { ... }; // error. 이미 모든 지역변수를 reference로 캡쳐 하는데 굳이 v2만 reference 캡쳐를?
							// 디폴트 모드와 다른 경우만 표기해야한다

	auto f8 = [x = v1, &y = v2](int a) { x, y };	// v1을 값 캡쳐, 람다표현식안에서는 x이름 사용
